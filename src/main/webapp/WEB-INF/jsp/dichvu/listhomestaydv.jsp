		...
		<table class="table table-striped" style="margin-bottom: 0">
			<thead>
				<tr>
					...
					<th style="min-width: 120px;">Người đặt</th>
			...
			<tbody>
				<c:forEach var="u" items="${danhsach}">
					<tr>
						...
						<td style="white-space: nowrap;"><fmt:formatNumber value="${u.tienCoc }" type="number" pattern="###,###" /> VNĐ</td>
						<td style="white-space: nowrap;"><fmt:formatDate pattern="HH:mm" value="${u.gioDat }" /> - <fmt:formatDate pattern="d/M/yyyy" value="${u.ngayDat }" /></td>
						<td style="white-space: nowrap;">${u.tenDangNhap}</td>
						<td style="white-space: nowrap;"><button type="button" class="btn btn-primary btn-sm" onclick="datdichvu(${u.maDatPhong },${soPhong })">Dịch vụ</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
