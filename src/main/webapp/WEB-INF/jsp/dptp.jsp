...
<div class="container">
	...
	<div class="container contaiStatus">
		<span style="opacity: 1;"><span onclick="dptppck()" class="margin-status ${activedptppck }"><b>Phòng có khách: </b>${phongCoKhach}</span><span onclick="dptppt()" class="margin-status ${activedptppt }"><b>Phòng trống: </b>${phongTrong}</span><span onclick="dptppdt()" class="margin-status ${activedptppdt }"><b>Phòng
					đặt trước: </b>${phongDatTruoc}</span><span onclick="dptptong()" class="margin-status ${activedptptong }"><b>Tổng: </b>${tongPhong}</span></span>
	
	<button class="btn btn-success btn-xs float-right sizebtn" onclick="dsdattruoc()">Xem toàn bộ danh sách đặt trước</button>
	</div>
</div>
						...
						<c:if test="${i==u.tang }">
							...
							>
								...
								<div id="thongtin-${u.maPhong}" class="thongtin ${u.trangThai==2?'thongtintt2':'thongtinttk2'}">
									...
									<div>
										<b>Giảm giá: </b>${u.khuyenMai } %
									</div>
									<c:if test="${u.trangThai==2}">
										<div>
											<b>Số người: </b>${u.countHomestay }
										</div>
									</c:if>
								</div>
