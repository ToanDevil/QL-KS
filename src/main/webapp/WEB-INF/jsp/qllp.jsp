  ...
  <c:if test="${not empty message }">
   ...
   <div class="container">
           ...
           <div class="input-group">
            <input class="form-control" type="text" name="data" placeholder="Tìm theo mã, tên" maxlength="100" value="${data }" required/>
		...
		<table class="table table-striped css-serial" style="white-space: nowrap;">
		<thead>
		<tr>
  ...
  <div class="modal fade" id="myModalsua${u.maLoaiPhong }" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
		...
		<div class="col-sm-10">
			<frm:input id="tenLoaiPhong" class="form-control" path="tenLoaiPhong" placeholder="Nhập tên loại phòng" value="${u.tenLoaiPhong }" maxlength="100" />
		</div>
	</div>
  <div class="form-group">
		<label for="moTa" style="font-weight: bold;">Mô tả:</label>
		<div class="col-sm-10">
			<frm:input id="moTa" class="form-control" path="moTa" placeholder="Nhập mô tả" value="${u.moTa }" maxlength="250"/>
		</div>
	</div>
	   ...
	   <table align="center">
         ...
         <tr>
            <td><frm:input type="hidden" path="maLoaiPhong" value="${maLoaiPhong }"/></td>
        	 ...
        	 <tr>
            <td> Tên loại phòng : </td><td><frm:input path="tenLoaiPhong" value="${tenLoaiPhong }" maxlength="100"/></td>
            </tr>
            <tr>
            <td> Mô tả : </td><td><frm:input path="moTa" value="${moTa }" maxlength="250"/></td>
            </tr>
            <tr>
...
<ul class="pagination" id="pagination" style="float: right;box-shadow: 1px 1px 5px #888888;">
	<li class="page-item first"><a href="lppage?data=${data }&page=${trangdau }" class="page-link">Trang đầu</a></li>
	<li class="page-item prev"><a href="lppage?data=${data }&page=<c:if test="${vitrihientai==1 }">${vitrihientai }</c:if><c:if test="${vitrihientai>1 }">${vitrihientai-1 }</c:if>" class="page-link"><</a></li>
	
	<c:forEach items="${listSoLuongTrang }" var="u">
	<li class="page-item"><a <c:if test="${u == vitrihientai}">style="background-color: rgba(0,0,0,.15);"</c:if>  href="lppage?data=${data }&page=${u }" class="page-link">${u }</a></li>
	</c:forEach>
	
	<li class="page-item next"><a href="lppage?data=${data }&page=<c:if test="${vitrihientai==trangcuoi }">${vitrihientai }</c:if><c:if test="${vitrihientai<trangcuoi }">${vitrihientai+1 }</c:if>" class="page-link">></a></li>
	<li class="page-item last"><a href="lppage?data=${data }&page=${trangcuoi }" class="page-link">Trang cuối</a></li>
</ul>
<br><br>
</c:if>
<c:if test="${ not empty danhsachtim}">
<ul class="pagination" id="pagination" style="float: right;box-shadow: 1px 1px 5px #888888;">
	<li class="page-item first"><a href="lppagetim?data=${data }&page=${trangdau }" class="page-link">Trang đầu</a></li>
	<li class="page-item prev"><a href="lppagetim?data=${data }&page=<c:if test="${vitrihientai==1 }">${vitrihientai }</c:if><c:if test="${vitrihientai>1 }">${vitrihientai-1 }</c:if>" class="page-link"><</a></li>
	
	<c:forEach items="${listSoLuongTrang }" var="u">
	<li class="page-item"><a <c:if test="${u == vitrihientai}">style="background-color: rgba(0,0,0,.15);"</c:if> href="lppagetim?data=${data }&page=${u }" class="page-link">${u }</a></li>
	</c:forEach>
	
	<li class="page-item next"><a href="lppagetim?data=${data }&page=<c:if test="${vitrihientai==trangcuoi }">${vitrihientai }</c:if><c:if test="${vitrihientai<trangcuoi }">${vitrihientai+1 }</c:if>" class="page-link">></a></li>
	<li class="page-item last"><a href="lppagetim?data=${data }&page=${trangcuoi }" class="page-link">Trang cuối</a></li>
</ul>
<br><br>
</c:if>
