    ...
    <strong>${message }</strong>
         ...
         <div class="container">
           ...
           <div class="input-group">
            <input class="form-control" type="number" name="data" placeholder="Tìm theo mã, số, tầng" maxlength="9" value="${data }" required/>
          ...
          <div style="overflow: auto;height: auto;width: 100%;box-shadow: 1px 1px 5px #888888;">
			<table class="table table-striped" style="font-size: 15px;white-space: nowrap;" >
					...
					<tr>
					<th style="max-width: 92px">Hình Ảnh</th>
						...
						<th style="min-width: 80px;vertical-align: middle;">Tiện Nghi</th>
						
						<th style="min-width: 80px;vertical-align: middle;">Giá Phòng / ngày</th>
						<th style="min-width: 80px;vertical-align: middle;">Giá giờ đầu</th>
						<th style="min-width: 80px;vertical-align: middle;">Giá giờ tiếp theo</th>
						<th style="min-width: 80px;vertical-align: middle;">Giá homestay</th>
					...
					<c:forEach var="u" items="${lPhongs}">
						...
						<td style="vertical-align: middle;"><img alt="" id="myImg${u.maPhong}" src="hinh/phong/${u.hinhAnh}" width="60px" height="60px"></td>
							...
							<td style="vertical-align: middle;"><fmt:formatNumber type="number" pattern="###,###" value="${u.giaPhongGioDau}" /> VNĐ</td>
							<td style="vertical-align: middle;"><fmt:formatNumber type="number" pattern="###,###" value="${u.giaPhongGioSau}" /> VNĐ</td>
							<td style="vertical-align: middle;"><fmt:formatNumber type="number" pattern="###,###" value="${u.giaHomestay}" /> VNĐ</td>
...
span.onclick = function() { 
  ...
  <div class="modal fade" id="myModalsua${u.maPhong}" role="dialog">
    ...
    <frm:input path="soPhong" class="form-control input-sm" id="sophong" type="number" placeholder="Nhập số phòng" value="${u.soPhong }" oninput="checkMaxLenghtNumber(this,9)" />
</div>
        
        <div class="form-group">
    <label for="tang" style="font-weight: bold;">Tầng</label>
    <frm:input path="tang" class="form-control input-sm" id="tang" type="number" placeholder="Nhập số  tầng" value="${u.tang }" oninput="checkMaxLenghtNumber(this,2)"/>
  </div>
   ...
   <c:if test="${u.tienNghi.contains('Vị trí tốt')}">document.getElementById("vitritot${u.maPhong }").checked = true;</c:if>
    ...
    <option value="${lp.maLoaiPhong }" label="${lp.tenLoaiPhong }" <c:if test="${u.loaiPhong.maLoaiPhong == lp.maLoaiPhong }">selected="selected"</c:if> />
    </c:forEach>
    </frm:select>
 ...
 <div class="form-group">
    ...
    <frm:input path="giaPhong" class="form-control input-sm" id="giaPhong" type="number" placeholder="Nhập giá phòng" value="${u.giaPhong }" oninput="checkMaxLenghtNumber(this,12)"/>
  </div>
  
  <div class="form-group">
    <label for="giaPhongGioDau" style="font-weight: bold;">Giá phòng giờ đầu:</label>
    <frm:input path="giaPhongGioDau" class="form-control input-sm" id="giaPhongGioDau" type="number" placeholder="Nhập giá phòng giờ đầu" value="${u.giaPhongGioDau }" oninput="checkMaxLenghtNumber(this,12)"/>
  </div>
  
  <div class="form-group">
    <label for="giaPhongGioSau" style="font-weight: bold;">Giá phòng giờ tiếp theo:</label>
    <frm:input path="giaPhongGioSau" class="form-control input-sm" id="giaPhongGioSau" type="number" placeholder="Nhập giá phòng giờ tiếp theo" value="${u.giaPhongGioSau }" oninput="checkMaxLenghtNumber(this,12)"/>
  </div>
  
   <div class="form-group">
    <label for="giaHomestay" style="font-weight: bold;">Giá homestay:</label>
    <frm:input path="giaHomestay" class="form-control input-sm" id="giaHomestay" type="number" placeholder="Nhập giá homestay" value="${u.giaHomestay }" oninput="checkMaxLenghtNumber(this,12)"/>
  </div>
 
 <div class="form-group">

    <label for="khuyenMai" style="font-weight: bold;">Giảm giá:</label>
    <frm:input path="khuyenMai" class="form-control input-sm" id="khuyenMai" type="number" placeholder="Nhập phần trăm giảm giá" min="0" max="100" value="${u.khuyenMai }" oninput="checkMaxLenghtNumber(this,3)"/>
  </div>
...
<ul class="pagination" id="pagination" style="float: right;box-shadow: 1px 1px 5px #888888;">
	<li class="page-item first"><a href="qlppage?data=${data }&page=${trangdau }" class="page-link">Trang đầu</a></li>
	<li class="page-item prev"><a href="qlppage?data=${data }&page=<c:if test="${vitrihientai==1 }">${vitrihientai }</c:if><c:if test="${vitrihientai>1 }">${vitrihientai-1 }</c:if>" class="page-link"><</a></li>
	
	<c:forEach items="${listSoLuongTrang }" var="u">
	<li class="page-item"><a <c:if test="${u == vitrihientai}">style="background-color: rgba(0,0,0,.15);"</c:if> href="qlppage?data=${data }&page=${u }" class="page-link">${u }</a></li>
	</c:forEach>
	
	<li class="page-item next"><a href="qlppage?data=${data }&page=<c:if test="${vitrihientai==trangcuoi }">${vitrihientai }</c:if><c:if test="${vitrihientai<trangcuoi }">${vitrihientai+1 }</c:if>" class="page-link">></a></li>
	<li class="page-item last"><a href="qlppage?data=${data }&page=${trangcuoi }" class="page-link">Trang cuối</a></li>
</ul>
<br><br>
</c:if>
 ...
 <br>
<ul class="pagination" id="pagination" style="float: right;box-shadow: 1px 1px 5px #888888;">
	<li class="page-item first"><a href="qlppagetim?data=${data }&page=${trangdau }" class="page-link">Trang đầu</a></li>
	<li class="page-item prev"><a href="qlppagetim?data=${data }&page=<c:if test="${vitrihientai==1 }">${vitrihientai }</c:if><c:if test="${vitrihientai>1 }">${vitrihientai-1 }</c:if>" class="page-link"><</a></li>
	
	<c:forEach items="${listSoLuongTrang }" var="u">
	<li class="page-item"><a <c:if test="${u == vitrihientai}">style="background-color: rgba(0,0,0,.15);"</c:if> href="qlppagetim?data=${data }&page=${u }" class="page-link">${u }</a></li>
	</c:forEach>
	
	<li class="page-item next"><a href="qlppagetim?data=${data }&page=<c:if test="${vitrihientai==trangcuoi }">${vitrihientai }</c:if><c:if test="${vitrihientai<trangcuoi }">${vitrihientai+1 }</c:if>" class="page-link">></a></li>
	<li class="page-item last"><a href="qlppagetim?data=${data }&page=${trangcuoi }" class="page-link">Trang cuối</a></li>
</ul>
<br><br>
</c:if>
