					...
					<c:forEach var="u" items="${l }">
						<c:if test="${i==u.phong.tang }">
							<div class="itemdptp">
								...
								> <img class="icondptp" src='<c:if test="${u.phong.trangThai==0 }">hinh/homegreen.png</c:if><c:if test="${u.phong.trangThai==1 }">hinh/homered.png</c:if><c:if test="${u.phong.trangThai==2 }">hinh/homecam.png</c:if>' >
									<div class="thongtinttk2">
										<div>
											<b>Phòng: </b>${u.phong.soPhong }
										</div>
										<div>
											<b>Loại: </b>
											...
											<c:if test="${u.phong.loaiPhong.tenLoaiPhong=='Thường'}">Thường</c:if>
										</div>
										<div>
											<b>Giảm giá: </b>${u.phong.khuyenMai } %
										</div>
										<div>
											<b>Trạng thái: </b>
											<c:if test="${u.phong.trangThai==0 }">Trống</c:if>
											<c:if test="${u.phong.trangThai==1}">Đã thuê</c:if>
										</div>
									</div>
								</a>
							</div>
